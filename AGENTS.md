...
- **Process Orchestration:** When destroying VPN tunnels, `server.py`'s `_kill_and_poll` uses native process polling tools (`pgrep` on Linux, `tasklist` on Windows) in a blocking loop to definitively verify that `gpclient` and `gpservice` have exited before reinitializing logic.
...
- **IPC Execution (`entrypoint.sh`):** Control endpoints rely on OS-agnostic local TCP sockets (`127.0.0.1:32801`, `127.0.0.1:32802`) as the primary production IPC standard, replacing legacy POSIX FIFOs. This guarantees stable cross-platform performance across both containerized Linux and native Windows execution environments. The listener endpoint (`control_listener.py`) must utilize a `while True` loop to act as a persistent daemon and buffer socket payloads until newline delimiters.
