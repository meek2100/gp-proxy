...
- **Cache Invalidation & Token Injection:** The cache-busting mechanism hashes the canonical bytes of all static assets to compute `build_hash`, and rewrites `index.html` to append `?v=<hash>` to asset URLs. This same sequence strictly injects the `EPHEMERAL_TOKEN` payload directly into the target meta tag.
- **Atomic API State:** Modifying global cryptographic properties like `_paired_pubkey` over HTTP handlers must strictly utilize a module-level lock (e.g. `_pairing_lock = threading.Lock()`) to prevent concurrent pairing race conditions on first-request hijacks.
