...
- **`backend/server.py`:** Python Control Server. Handles optional `API_TOKEN` bearer auth logic, length-limited payload parsing, uses Python 3.8+ Walrus operators to securely map API elements, reads binary `CLIENT_LOG`, and hosts the UDP Beacon. Control endpoints rely on OS-agnostic local TCP sockets (`IPC_CONTROL_PORT` and `IPC_STDIN_PORT`). Process lifecycle management (`_kill_and_poll`) dynamically detects the host OS (`sys.platform == "win32"`) to utilize `taskkill`, enabling full native Windows development and testing outside the container via cross-platform canonical path resolution (`pathlib.Path.resolve()`). Contains an explicit `str()` cast guard in `log_message` to prevent Python 3.14+ `HTTPStatus` enum objects from crashing the polling loop handler during syntax parsing errors.
...
- **Process Orchestration:** When destroying VPN tunnels, `server.py`'s `_kill_and_poll` dynamically determines container privileges to invoke `pkill` and `pgrep` safely. It relies on a blocking loop alongside `subprocess.run(..., capture_output=True)` to natively return byte streams that satisfy strict Mypy types, verifying that processes have exited before reinitializing logic. If processes refuse to terminate gracefully within the polling window, it automatically escalates to a `SIGKILL` (-9) payload to prevent zombie state races. _Note: If `gpservice` necessitates passwordless `sudo` privileges configured in the Dockerfile under a limited user profile, the script will automatically detect and apply the `sudo` prefix._
...
- **Cache Invalidation:** `server.py` injects MD5 cache-busting query strings directly into `index.html` and `index.js` at runtime for all configured assets (`.png`, `.svg`, `.css`, etc.). Caching headers must therefore be left as `immutable` to preserve bandwidth, as the injected hash implicitly guarantees cache breaking on new container releases without relying on browser heuristics.
