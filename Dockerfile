...
RUN useradd -m -s /bin/bash gpuser && \
    ...
    "Cmnd_Alias GP_PROCCTL = /usr/bin/pkill -x gpclient, /usr/bin/pkill -x gpservice, /usr/bin/pkill -x gost, /usr/bin/pkill -9 -x gpclient, /usr/bin/pkill -9 -x gpservice, /usr/bin/pkill -9 -x gost, /usr/bin/pgrep -x gpclient, /usr/bin/pgrep -x gpservice, /usr/bin/pgrep -x gost" \
